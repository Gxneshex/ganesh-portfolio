    ...
    <section id="experience" className="py-24 px-4">
      <div ref={ref} className={`max-w-4xl mx-auto transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <h2 className="text-3xl sm:text-4xl font-bold mb-2">
          <span className="bg-gradient-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent">Experience</span>
        </h2>
        <div className="w-20 h-1 bg-gradient-to-r from-indigo-500 to-cyan-500 rounded mb-12" />

        <div className="space-y-8">
          {portfolioData.experience.map((exp, i) => (
            <div key={i} className="relative pl-8 border-l-2 border-slate-700 hover:border-indigo-500 transition-colors">
              <div className="absolute -left-2.5 top-0 w-5 h-5 rounded-full bg-slate-800 border-2 border-indigo-500" />
              ...
              <div className="bg-slate-800/30 rounded-xl p-6 border border-slate-700 hover:border-slate-600 transition-all">
                ...
                <div className="mb-4">
                  <h4 className="text-slate-400 text-sm uppercase tracking-wider mb-2">Key Highlights</h4>
                ...
                <div className="flex flex-wrap gap-2">
                  {exp.technologies.map((tech, k) => (
                    <span key={k} className="px-2.5 py-1 bg-slate-700/50 text-slate-300 text-xs rounded-md">
                      {tech}
                    </span>
                  ))}
                </div>
